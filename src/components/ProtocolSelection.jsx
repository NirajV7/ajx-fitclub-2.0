...
const ProtocolSelection = ({ onClose }) => {
    ...
    // RESTORED: Original Plan Selection and Data
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-[#050505] flex flex-col antialiased selection:bg-[#ccff00] selection:text-black">
            ...
            {/* --- MAIN BODY: ORIGINAL CARD STYLING & DATA --- */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 max-w-7xl mx-auto">
                    {deploymentTiers.map((plan, index) => (
                        ...
                        >
                            ...
                            {/* RESTORED: Original Black Italic Fonts */}
                            <h3 className="text-lg font-black italic uppercase mb-1 tracking-tight text-white">{plan.name}</h3>
                            <span className="text-3xl font-black italic tracking-tighter text-white mb-6">
                                {plan.price}
                                <span className="text-xs text-white/20 font-bold tracking-normal italic ml-2">/mo</span>
                            </span>

                            {/* Original Feature List Styling */}
                            <div className="space-y-3 mb-4">
                                {plan.features.map((feature, fIndex) => (
                                    <div key={fIndex} className="flex items-center gap-2">
                                        <div className={`w-1 h-1 rounded-full ${selectedPlan === plan.name ? 'bg-[#ccff00]' : 'bg-white/20'}`}></div>
                                        <span className={`text-[10px] font-bold uppercase tracking-widest ${selectedPlan === plan.name ? 'text-white' : 'text-white/40'}`}>
                                            {feature}
                                        </span>
                                    </div>
                                ))}
