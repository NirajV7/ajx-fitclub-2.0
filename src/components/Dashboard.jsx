...
const Dashboard = () => {
    ...
    // Get today's date in YYYY-MM-DD format to prevent future DOB selection
    ...
    // Logic to revert local state to database values
    const handleReset = () => {
        if (userData?.assessment) {
            setBioData({
                gender: userData.assessment.gender || '',
                dob: userData.assessment.dob || '',
                healthIssues: userData.assessment.healthIssues || ''
            });
        } else {
            setBioData({ gender: '', dob: '', healthIssues: '' });
        }
    };
                ...
                {showBioSync && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-6 bg-black/90 backdrop-blur-xl">
                        <div className="w-full max-w-md bg-[#0f0f0f] border border-white/10 rounded-[32px] p-6 md:p-10 relative animate-in fade-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto scrollbar-hide">
                            {/* Reset state when closing modal manually */}
                            <button
                                onClick={() => { setShowBioSync(false); handleReset(); }}
                            ...
                            <form onSubmit={handleBioSubmit} className="space-y-6">
                                ...
                                {/* Reset and Save Action Group */}
                                <div className="grid grid-cols-2 gap-4">
                                    <button
                                        type="button"
                                        onClick={handleReset}
                                        className="py-4.5 bg-white/5 border border-white/10 text-white/40 hover:text-white font-bold uppercase tracking-widest text-[10px] rounded-2xl transition-all"
                                    >
                                        Reset
                                    </button>
                                    <button
                                        ...
                                        type="submit"
                                        className="py-4.5 bg-[#ccff00] text-black font-bold uppercase tracking-widest text-[10px] rounded-2xl shadow-lg active:scale-95 transition-all disabled:opacity-30"
                                    >
