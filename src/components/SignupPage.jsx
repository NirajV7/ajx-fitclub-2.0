...
const SignupPage = () => {
    ...
    const isFormValid = formData.firstName.trim().length >= 2 && formData.lastName.trim().length >= 1 && isEmailValid && codesMatch;
                ...
                <div className="max-w-xl w-full">
                    <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/10 p-6 md:p-10 rounded-[32px] relative overflow-hidden shadow-2xl">
                        ...
                        <div className="mb-6 text-center">
                            <div className="inline-flex items-center gap-2 px-3 py-1 border border-[#ccff00]/30 rounded-full bg-[#ccff00]/5 mb-4">
                                ...
                                <span className="text-[8px] font-black uppercase tracking-[0.3em] text-[#ccff00]">Recruitment Protocol 4.02</span>
                            </div>
                        ...
                        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5 group">
                                    ...
                                    <input type="text" value={formData.firstName} onChange={(e) => handleNameChange(e, 'firstName')} placeholder="ALEX" className="w-full bg-white/[0.05] border border-white/10 rounded-xl py-3.5 px-4 text-white font-bold text-[11px] uppercase placeholder:text-white/40 focus:border-[#ccff00]/40 outline-none transition-all" />
                                </div>
                                <div className="space-y-1.5 group">
                                    ...
                                    <input type="text" value={formData.lastName} onChange={(e) => handleNameChange(e, 'lastName')} placeholder="DOE" className="w-full bg-white/[0.05] border border-white/10 rounded-xl py-3.5 px-4 text-white font-bold text-[11px] uppercase placeholder:text-white/40 focus:border-[#ccff00]/40 outline-none transition-all" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-1.5 group">
                                ...
                                <div className="relative">
                                    ...
                                    <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="ALEX.DOE@GMAIL.COM" className={`w-full bg-white/[0.05] border rounded-xl py-3.5 pl-12 pr-4 text-white font-bold text-[11px] uppercase placeholder:text-white/40 focus:border-[#ccff00]/40 outline-none transition-all ${formData.email.length > 0 && !isEmailValid ? 'border-red-500/30' : 'border-white/10'}`} />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5 group">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="password" inputMode="numeric" value={formData.code} onChange={(e) => handleCodeChange(e, 'code')} placeholder="0000" className="w-full bg-white/[0.05] border border-white/10 rounded-xl py-3.5 pl-10 pr-4 text-white font-bold text-[11px] placeholder:text-white/40 focus:border-[#ccff00]/40 outline-none transition-all" />
                                    </div>
                                </div>
                                <div className="space-y-1.5 group">
                                    <span className={`text-[8px] font-black uppercase transition-colors ${codesMatch ? 'text-[#ccff00]' : 'text-white/40'}`}>[ VERIFY_KEY ]</span>
                                    <input type="password" inputMode="numeric" value={formData.confirmCode} onChange={(e) => handleCodeChange(e, 'confirmCode')} placeholder="0000" className={`w-full bg-white/[0.05] border rounded-xl py-3.5 px-4 text-white font-bold text-[11px] placeholder:text-white/40 outline-none transition-all duration-300 ${codesMatch ? 'border-[#ccff00] shadow-[0_0_15px_rgba(204,255,0,0.2)]' : 'border-white/10'}`} />
                                </div>
                            </div>

                            <button disabled={!isFormValid} className={`group relative w-full flex items-center justify-center gap-3 py-4 font-black uppercase tracking-[0.3em] text-[11px] rounded-2xl transition-all duration-700 overflow-hidden shadow-2xl active:scale-[0.98] ${isFormValid ? 'bg-white text-black hover:bg-[#ccff00]' : 'bg-white/5 text-white/20 cursor-not-allowed border border-white/5'}`}>
                                <span className="relative z-10">{isFormValid ? 'Start Transformation' : 'Awaiting Full Auth'}</span>
                    ...
                    <div className="mt-6 flex justify-between items-center px-4 opacity-40">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[7px] font-mono text-white uppercase tracking-widest">Auth Protocol: {isFormValid ? 'READY' : 'PENDING'}</span>
                        </div>
