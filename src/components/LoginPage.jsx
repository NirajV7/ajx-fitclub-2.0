...
const SigninPage = () => {
    ...
    // UPDATED GUARD: Prevent the loop by checking the intent flag
    useEffect(() => {
        if (user && !authLoading) {
            const isActiveAuth = sessionStorage.getItem('ajx_auth_active');

            if (isActiveAuth) {
                sessionStorage.removeItem('ajx_auth_active');
                navigate("/dashboard", { replace: true });
            } else {
                // User hit "Back" from dashboard. Skip the login terminal and go Home.
    ...
    const handleAuth = async (e) => {
        ...
        try {
            ...
            // Set the flag so the guard allows entry to the terminal
    ...
    if (authLoading || user) {
        return (
            <div className="h-screen bg-black flex flex-col items-center justify-center">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white/40 animate-pulse font-black">
    ...
    return (
        <div className="h-screen flex flex-col bg-black text-white font-sans selection:bg-[#ccff00] selection:text-black antialiased relative overflow-hidden font-black">

            <nav className="shrink-0 w-full z-[100] bg-black/95 backdrop-blur-xl py-8 border-b border-white/5">
                <div className="max-w-7xl mx-auto px-6 md:px-8 flex items-center justify-between relative font-bold text-white font-black">
                    <MobileMenuToggle side="left" />
                    <div className="flex-1 md:flex-none flex justify-center md:justify-start font-bold text-white">
                        <BrandLogo />
                    </div>
                    <div className="hidden md:flex items-center space-x-12 text-[10px] font-bold tracking-[0.3em] uppercase font-black font-bold text-white font-black">
                        <Link to="/" className="hover:text-[#ccff00] transition-colors tracking-widest text-white font-bold relative group/link uppercase font-black">
                            Home
                            <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-[#ccff00] group-hover/link:w-full transition-all font-bold"></span>
                        </Link>
                        <Link to="/signup" className="px-6 py-2 border border-white/20 text-white rounded-full hover:border-[#ccff00] hover:text-[#ccff00] transition-all duration-500 font-black tracking-widest text-[10px] uppercase font-bold font-black">
                            New Recruit?
                        </Link>
                    </div>
            ...
            <main className="flex-grow flex items-center justify-center px-4 relative z-10 overflow-hidden font-bold text-white font-black">
                <div className="max-w-xl w-full">
                    <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/10 p-6 md:p-10 rounded-[32px] relative overflow-hidden shadow-2xl">
                        ...
                        <div className="mb-8 text-center font-black">
                            ...
                            <h1 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter text-white mb-1 text-balance font-bold font-black">Authenticate.</h1>
                            <p className="text-white/40 text-[9px] font-bold tracking-[0.2em] uppercase leading-relaxed font-black">Enter your credentials to access the terminal</p>
                        </div>

                        <form className="space-y-6" onSubmit={handleAuth}>
                            <div className="space-y-2 group font-bold font-black">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className={`w-full bg-white/[0.05] border rounded-xl py-4 pl-12 pr-4 text-white font-bold text-[11px] uppercase placeholder:text-white/20 focus:border-[#ccff00]/40 outline-none transition-all font-bold ${formData.email.length > 0 && !isEmailValid ? 'border-red-500/30' : 'border-white/10'}`}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 group font-bold font-black">
                                <div className="flex justify-between items-center">
                                    ...
                                    {isCodeComplete && <span className="text-[7px] text-[#ccff00] font-bold uppercase tracking-widest animate-pulse font-black">Key Ready</span>}
                                </div>
                                <div className="relative font-black">
                                    ...
                                    <input
                                        ...
                                        placeholder="0000"
                                        className={`w-full bg-white/[0.05] border rounded-xl py-4 pl-12 pr-4 text-white font-bold text-[13px] tracking-[0.5em] placeholder:text-white/20 focus:border-[#ccff00]/40 outline-none transition-all font-bold ${isCodeComplete ? 'border-[#ccff00] shadow-[0_0_15px_rgba(204,255,0,0.1)]' : 'border-white/10'}`}
                                    />
                            ...
                            <button
                                ...
                                className={`group relative w-full flex items-center justify-center gap-3 py-5 font-black uppercase tracking-[0.4em] text-[11px] rounded-2xl transition-all duration-700 overflow-hidden shadow-2xl active:scale-[0.98] ${isAuthReady ? 'bg-white text-black hover:bg-[#ccff00]' : 'bg-white/5 text-white/20 cursor-not-allowed border border-white/5 font-bold font-black font-black font-black'}`}
                            >
                                <span className="relative z-10 font-bold font-black">
                            ...
                            <div className="flex flex-col items-center gap-6 mt-4 font-bold text-white font-black">
                                <button type="button" className="text-[9px] font-bold uppercase tracking-widest text-white/30 hover:text-white transition-colors underline-offset-4 hover:underline font-black">
                                    Forgot Security Key?
                                </button>

                                <div className="w-full pt-6 border-t border-white/5 flex flex-col items-center font-bold text-white font-black">
                                    <p className="text-[8px] font-black uppercase tracking-[0.2em] text-white/20 mb-3 italic">Not yet recognized?</p>
                                    <Link to="/signup" className="text-[10px] font-black uppercase tracking-[0.3em] text-[#ccff00] hover:text-white transition-all flex items-center gap-2 group/link font-bold font-black">
                                        Initialize Profile
                    ...
                    <div className="mt-8 flex justify-between items-center px-4 opacity-40 font-bold text-white font-black">
                        <div className="flex items-center gap-2">
                            <Cpu size={10} className="text-[#ccff00] animate-pulse font-black" />
                        ...
                        <span className="text-[7px] font-mono text-white uppercase tracking-widest font-bold font-black">TERMINAL: AJX.SIG.04</span>
                    </div>
                </div>
            </main>
        </div>
