...
import { MapPin, Shield, Wind, ArrowRight, X, Check, Phone, MessageCircle, Monitor, Video, Home, User, Lock, Award, ChevronDown, Activity } from 'lucide-react';
import {Link} from 'react-router-dom';

// Custom Instagram Icon component - Fixed duplicate attribute error
...
// --- REFINED LOGO: ONE LINE & CENTERED ON MOBILE, STACKED ON DESKTOP ---
// Added onClick prop to handle scroll-to-top functionality
    ...
    >
      ...
      <span className="text-lg md:text-2xl font-black italic tracking-tighter text-white leading-none block transition-all duration-500 group-hover:tracking-normal">
        AJX
            ...
            {/* Subtle underline indicator - only on desktop stacked version */}
...
const LandingPage = () => {
    ...
    // Function to scroll the website to the top smoothly
    const scrollToTop = () => {
    ...
    return (
        <div className="min-h-screen bg-black text-white font-sans selection:bg-[#ccff00] selection:text-black antialiased relative overflow-x-hidden">
            {/* --- LIVING ELEMENT: BACKGROUND EMBERS --- */}
      ...
      `}</style>
            ...
            <nav className={`fixed w-full z-[100] transition-all duration-700 ${isScrolled ? 'bg-black/95 backdrop-blur-xl py-4 border-b border-white/5 shadow-[0_10px_30px_rgba(0,0,0,0.5)]' : 'bg-transparent py-8'}`}>
                <div className="max-w-7xl mx-auto px-6 md:px-8 flex items-center justify-between relative">
                    ...
                    {/* Logo - One line and Centered on Mobile - Added scrollToTop function */}
                    ...
                    <div className="hidden md:flex items-center space-x-12 text-[10px] font-bold tracking-[0.3em] uppercase">
                        {['About', 'Pillars', 'Plans', 'Experience'].map((item) => (
                            <a key={item} href={`#${item.toLowerCase()}`} className="hover:text-[#ccff00] active:text-[#ccff00] transition-colors cursor-pointer tracking-widest uppercase text-white font-bold relative group/link">
                                {item}
            ...
            <span className="relative flex h-2 w-2">
              ...
              <span className="relative inline-flex rounded-full h-2 w-2 bg-[#ccff00]"></span>
                        ...
                        <span className="text-[10px] font-bold tracking-[0.2em] uppercase opacity-80 text-white font-black italic">DPIIT Recognized Startup</span>
                    </div>

                    <h1 className="text-7xl md:text-9xl font-black tracking-tighter mb-6 leading-[0.85] uppercase italic text-white font-bold drop-shadow-2xl">
                        ACCESS.<br/>
                        JOIN.<br/>
                    ...
                    <div className="grid lg:grid-cols-3 gap-px bg-white/10 border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
                        {pillars.map((pillar, index) => (
                            <div key={index} className="group bg-black p-12 lg:p-16 transition-all duration-700 hover:bg-neutral-900/50 active:bg-neutral-900 relative overflow-hidden">
                                <div className="absolute top-0 left-0 w-full h-1 bg-[#ccff00] scale-x-0 group-hover:scale-x-100 group-active:scale-x-100 transition-transform duration-700 origin-left shadow-[0_0_15px_rgba(204,255,0,0.5)]" />
                                <div className="mb-12 text-white/30 group-hover:text-[#ccff00] group-active:text-[#ccff00] transition-colors duration-500 transform group-hover:scale-110 group-hover:-rotate-12">
                                    {pillar.icon}
                                </div>
                                <h3 className="text-3xl font-black mb-6 tracking-tighter italic group-hover:text-[#ccff00] group-active:text-[#ccff00] transition-colors text-white uppercase">{pillar.title}</h3>
                                <p className="text-white/40 leading-relaxed text-lg font-light group-hover:text-white/70 transition-colors duration-500">
                                    {pillar.description}
                                </p>
                            </div>
                        ))}
            ...
            {/* Plans Section */}
            <section id="plans" className="py-12 md:py-20 px-6 md:px-8 bg-black text-white">
                <div className="max-w-7xl mx-auto">
                    <div className="text-center mb-12 md:mb-16">
                        ...
                        <p className="text-[#ccff00] uppercase tracking-widest text-xs font-bold italic tracking-[0.3em]">First session trial available</p>
                    </div>
                    ...
                    >
                        {plans.map((plan, index) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-8 text-white font-bold">
                    ...
                    {plan.tag}
                                ...
                                <h3 className="text-2xl font-black tracking-tighter mb-2 italic leading-tight uppercase text-white font-bold">{plan.name}</h3>
                                <p className="text-sm mb-8 font-medium text-white/40">{plan.subtitle}</p>

                                <div className="flex items-baseline gap-1 mb-10">
                                    <span className="text-5xl font-black tracking-tighter text-[#ccff00] lg:text-white group-hover:text-[#ccff00] transition-colors duration-500">{plan.price}</span>
                                    <span className="text-sm font-bold uppercase tracking-widest text-white/30">{plan.period}</span>
                                </div>

                                <div className="space-y-4 mb-12 flex-grow text-white font-bold">
                                ...
                                <button className="w-full py-5 rounded-full font-black uppercase tracking-widest text-[10px] bg-white text-black transition-all hover:bg-[#ccff00] active:bg-[#ccff00] active:scale-90 shadow-[0_4px_20px_rgba(204,255,0,0.1)]">
                                    {plan.buttonText}
                                </button>
                            </div>
                        ))}
                    ...
                    <div className="flex justify-center items-center gap-3 mt-10 lg:hidden">
                        {plans.map((_, i) => (
                            <div
                                key={i}
                                className={`h-1.5 rounded-full transition-all duration-500 ease-out ${activePlanIndex === i ? 'w-10 bg-[#ccff00] shadow-[0_0_15px_rgba(204,255,0,0.6)]' : 'w-2.5 bg-white/20'}`}
                            />
                        ))}
                    </div>

                    <div className="mt-16 mb-4 flex flex-col items-center">
                        <div className="w-px h-12 bg-[#ccff00]/20"></div>
                        <div className="w-1 h-1 rounded-full bg-[#ccff00]/40"></div>
                    </div>

                    {/* COACH SECTION */}
                    <div className="p-6 md:p-12 rounded-[40px] border border-white/5 bg-neutral-900/20 group hover:border-[#ccff00]/20 transition-all duration-700 overflow-hidden relative text-white font-bold">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-[#ccff00]/5 rounded-full blur-[100px] -mr-32 -mt-32"></div>

                        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-16 text-white font-bold">
                            ...
                            <div className="flex-grow text-center md:text-left w-full">
                                ...
                                <h4 className="text-4xl md:text-6xl font-black italic uppercase mb-6 tracking-tighter leading-none text-white font-bold">Coach Athul</h4>
                                ...
                                <div className="flex items-center justify-center md:justify-start gap-4 md:gap-6 text-white font-bold">
                                    <a href="https://wa.me/917736720936" className="group flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-2xl border border-[#ccff00]/40 lg:border-[#ccff00]/30 text-[#ccff00] transition-all duration-500 hover:bg-[#ccff00] hover:text-black active:bg-[#ccff00] active:text-black active:scale-90 hover:scale-110 hover:-rotate-6 shadow-[0_0_15px_rgba(204,255,0,0.1)] hover:shadow-[0_0_25px_rgba(204,255,0,0.3)]">
                                        <MessageCircle size={22} />
                                    </a>
                                    <a href="#" className="group flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-2xl border border-[#ccff00]/40 lg:border-[#ccff00]/30 text-[#ccff00] transition-all duration-500 hover:bg-[#ccff00] hover:text-black active:bg-[#ccff00] active:text-black active:scale-90 hover:scale-110 hover:-rotate-6 shadow-[0_0_15px_rgba(204,255,0,0.1)] hover:shadow-[0_0_25px_rgba(204,255,0,0.3)]">
                                        <InstagramIcon size={22} />
                                    </a>
                                    <a href="tel:+917736720936" className="group flex items-center justify-center w-12 h-12 md:w-14 md:h-14 rounded-2xl border border-[#ccff00]/40 lg:border-[#ccff00]/30 text-[#ccff00] transition-all duration-500 hover:bg-[#ccff00] hover:text-black active:bg-[#ccff00] active:text-black active:scale-90 hover:scale-110 hover:-rotate-6 shadow-[0_0_15px_rgba(204,255,0,0.1)] hover:shadow-[0_0_25px_rgba(204,255,0,0.3)]">
                                        <Phone size={22} />
                                    </a>
                                </div>
                            </div>
            ...
            <section id="about" className="py-24 md:py-40 px-6 md:px-8 relative overflow-hidden bg-black text-white font-bold">
                    ...
                    <div className="flex flex-col items-start gap-12 md:gap-20">
                        ...
                        <div className="group relative w-full p-8 md:p-12 bg-white/5 border border-white/10 rounded-[32px] overflow-hidden backdrop-blur-xl transition-all duration-700 hover:border-[#ccff00]/30">
                            ...
                            <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
                                ...
                                <p className="text-sm md:text-base text-white/50 font-medium tracking-wide leading-relaxed max-w-xl uppercase italic font-bold">
                        ...
                        <div className="flex items-center gap-4 w-full">
                            ...
                            <p className="text-[10px] md:text-xs font-black tracking-[0.5em] text-[#ccff00] uppercase italic">
                ...
                <div className="mt-24 md:mt-40 flex flex-col items-center">
                    <div className="w-px h-12 bg-[#ccff00]/10"></div>
                    <div className="w-6 h-px bg-[#ccff00]/20"></div>
                </div>
            </section>
            ...
            <footer id="experience" className="relative pt-32 pb-20 px-8 bg-black overflow-hidden border-t border-white/5 text-white font-bold">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-full bg-gradient-to-b from-[#ccff00]/20 via-[#ccff00]/5 to-transparent opacity-40"></div>
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                    <div className="text-center mb-24">
                        ...
                        <h2 className="text-[12vw] lg:text-[10vw] font-black tracking-tighter italic uppercase leading-[0.8] mb-16 opacity-90 transition-all hover:opacity-100 hover:scale-[1.02] duration-700 cursor-default text-white font-bold">
                            JOIN THE <br/>
                            <span className="text-[#ccff00]">COLLECTIVE.</span>
                        </h2>

                        <div className="flex flex-col md:flex-row items-stretch justify-center gap-6 max-w-4xl mx-auto px-4 text-white">
                            <div className="flex-1 group relative p-1 bg-white/5 rounded-[32px] overflow-hidden transition-all hover:bg-white/10">
                                <div className="absolute -inset-1 bg-gradient-to-r from-transparent via-[#ccff00]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                <div className="relative p-8 h-full flex flex-col items-center justify-between border border-white/10 rounded-[30px] bg-black/40 backdrop-blur-xl">
                                    <div className="mb-6 text-center">
                                        ...
                                        <h3 className="text-2xl font-black italic uppercase tracking-tighter mb-2 text-white font-black uppercase tracking-widest font-bold">New Recruit</h3>
                                    ...
                                    <Link to="/signup" className="w-full">
                                        <button className="w-full py-5 bg-white text-black font-black uppercase tracking-[0.4em] text-[10px] rounded-2xl hover:bg-[#ccff00] active:scale-95 transition-all shadow-xl font-bold">
                                            Create Profile
                                        </button>
                                    </Link>
                                </div>
                            ...
                            <div className="flex-1 group relative p-1 bg-white/5 rounded-[32px] overflow-hidden transition-all hover:bg-white/10">
                                <div className="relative p-8 h-full flex flex-col items-center justify-between border border-white/10 rounded-[30px] bg-black/40 backdrop-blur-xl text-white font-bold">
                                    <div className="mb-6 text-center">
                                        <Lock className="w-10 h-10 text-white mb-4 mx-auto opacity-40" />
                                        <h3 className="text-2xl font-black italic uppercase tracking-tighter mb-2 text-white font-black uppercase tracking-widest font-bold">Active Member</h3>
                                    ...
                                    <Link to="/login" className="w-full">
                                    <button className="w-full py-5 border border-white/20 text-white font-black uppercase tracking-[0.4em] text-[10px] rounded-2xl hover:bg-white hover:text-black active:scale-95 transition-all font-black uppercase tracking-widest text-[10px] font-bold">
                                        Secure Log In
                                    </button>
                                    </Link>
                                </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-12 pt-16 border-t border-white/5 text-white font-bold uppercase relative">
                        <div className="flex-1 md:flex-none flex justify-center md:justify-start">
                            {/* Added scrollToTop here as well for consistency */}
                            <BrandLogo onClick={scrollToTop} />
                        </div>
            ...
            {/* Symmetrical Mobile Full-Screen Menu */}
            {mobileMenuOpen && (
                <div className="fixed inset-0 z-[1000] bg-black/98 backdrop-blur-3xl p-8 flex flex-col justify-between animate-in slide-in-from-right duration-500 overflow-hidden text-white font-bold">
                    {/* Animated Background Pulse */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] rotate-12 opacity-[0.03] select-none pointer-events-none text-[80vw] font-black italic text-white font-bold">
                        AJX
                    </div>

                    <div className="absolute top-1/4 -right-20 w-80 h-80 bg-[#ccff00]/5 rounded-full blur-[120px] text-white font-bold"></div>
                    <div className="absolute bottom-1/4 -left-20 w-80 h-80 bg-[#ccff00]/5 rounded-full blur-[120px] text-white font-bold"></div>

                    {/* Menu Header (Consistent with Main Nav) */}
                    ...
                    {/* UNIQUE HUD MENU GRID */}
                    <div className="relative z-10 grid grid-cols-2 gap-4 py-12 text-white font-bold">
                        <div className="absolute inset-x-0 h-[2px] bg-gradient-to-r from-transparent via-[#ccff00]/30 to-transparent scan-line opacity-50 z-0"></div>
                        ...
                        {[
                            ...
                            <a
                                ...
                                className="group relative flex flex-col items-start p-6 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-xl overflow-hidden active:bg-[#ccff00]/10 transition-all duration-300"
                            >
                                <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-[#ccff00] opacity-0 group-active:opacity-100"></div>
                                <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-[#ccff00] opacity-0 group-active:opacity-100"></div>
                    ...
                    <div className="relative z-10 flex flex-col gap-8 items-center border-t border-white/10 pt-12">
                        <div className="flex gap-12">
                            <a href="https://wa.me/917736720936" className="text-white hover:text-[#ccff00] active:scale-110 transition-all font-bold text-white"><MessageCircle size={28} /></a>
                            <a href="#" className="text-white hover:text-[#ccff00] active:scale-110 transition-all font-bold text-white"><InstagramIcon size={28} /></a>
                            <a href="tel:+917736720936" className="text-white hover:text-[#ccff00] active:scale-110 transition-all font-bold text-white"><Phone size={28} /></a>
                        </div>

                        <div className="flex flex-col items-center gap-2">
                            <div className="w-12 h-[1px] bg-white/10"></div>
                            <div className="text-[8px] font-black tracking-[0.6em] uppercase text-white/30 italic font-black uppercase tracking-widest font-bold">
                                Fitness Redefined
                            </div>
                        </div>
                    </div>
